...
MASTER_PORT="${TRITON_GIN_RDMA_MASTER_PORT:-29790}"
RANK_SIZE=$((NNODES * LOCAL_RANKS))
BASE_RANK=$((NODE_RANK * LOCAL_RANKS))
BASE_TAG="${TRITON_GIN_RDMA_TAG:-rdma_collective_$(date +%Y%m%d_%H%M%S)}"
LOG_ROOT="/home/kaixin/triton_gin_rdma_collective_${BASE_TAG}_node${NODE_RANK}"
...
if [ "$RANK_SIZE" -ne 2 ]; then
  echo "RDMA collective smoke currently expects exactly 2 ranks; got ${RANK_SIZE}" >&2
  exit 1
fi
...
for local_rank in $(seq 0 $((LOCAL_RANKS - 1))); do
  ...
  tail -n 240 "${LOG_ROOT}/rank${global_rank}.log" || true
done

if [ "$failed" -ne 0 ]; then
  echo "GIN HCCL RDMA collective node ${NODE_RANK}: FAILED; logs in ${LOG_ROOT}" >&2
  exit 1
fi

echo "GIN HCCL RDMA collective node ${NODE_RANK}: PASS"
