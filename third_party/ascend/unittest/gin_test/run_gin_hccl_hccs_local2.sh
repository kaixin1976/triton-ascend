...
BASE_TAG="${TRITON_GIN_HCCS_TAG:-hccs_$(date +%Y%m%d_%H%M%S)}"
LOG_ROOT="/home/kaixin/triton_gin_hccs_${BASE_TAG}"
...
run_pair() {
  ...
  tail -n 100 "${log_dir}/rank0.log" || true
  tail -n 100 "${log_dir}/rank1.log" || true
...
run_pair hccs_channel_probe "${SCRIPT_DIR}/ascend_gin_hccl_channel_probe_run.py" \
  --hccl-channel-engine "$HCCL_ENGINE" --hccl-op-expansion-mode "$HCCL_OP_MODE" \
  --require-hccs --require-acquire
run_pair hccs_ops "${SCRIPT_DIR}/ascend_gin_op_coverage_run.py" \
  "${COMMON_HCCL_ARGS[@]}" --n 128 --block 32
run_pair hccs_fused_allgather "${SCRIPT_DIR}/ascend_gin_fused_all_gather_run.py" \
  "${COMMON_HCCL_ARGS[@]}" --n 128 --block 32
run_pair hccs_collective_patterns "${SCRIPT_DIR}/ascend_gin_collective_patterns_run.py" \
  "${COMMON_HCCL_ARGS[@]}" --case all --n 128 --block 32

echo "GIN HCCL HCCS local2: PASS"
